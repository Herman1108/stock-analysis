"""
Script to add Company Profile data to PANI Excel file
...
row += 1
profile_data = [
    ('Nama Emiten', 'PT Pantai Indah Kapuk Dua Tbk'),
    ('Kode Saham', 'PANI'),
    ('Papan Pencatatan', 'Papan Utama (Main Board)'),
    ('Sektor Usaha', 'Properti & Real Estat'),
    ('Sub Sektor', 'Properti & Real Estat'),
    ('Bidang Industri', 'Pengembangan & Pengelolaan Real Estat'),
    ('Aktivitas Bisnis', 'Perindustrian, aktivitas holding, dan konsultasi manajemen'),
]
...
row += 1
history_data = [
    ('Tanggal Pencatatan', '18 September 2018'),
    ('Tanggal Efektif', '06 September 2018'),
    ('Nilai Nominal', 'Rp 100'),
    ('Harga Penawaran Perdana', 'Rp 108'),
    ('Jumlah Saham IPO', '150.000.000 lembar'),
    ('Dana IPO Terhimpun', 'Rp 16,20 Miliar'),
    ('Penjamin Emisi', 'PT Investindo Nusantara Sekuritas'),
...
background_text = [
    'Emiten ini merupakan transformasi dari PT Pratama Abadi Nusa Industri Tbk.',
    'Awal mula berdiri pada 1 Maret 2001 sebagai produsen kemasan kaleng.',
    'Tahun 2017, melakukan ekspansi dengan mengakuisisi perusahaan pengolahan',
    'hasil laut dan jasa penyimpanan dingin (cold storage).',
    'Bertransformasi menjadi perusahaan terbuka pada September 2018.',
    'Saat ini fokus pada pengembangan properti dan real estat premium.',
]
...
row += 1
ws.cell(row=row, column=start_col, value='Per 31 Desember 2025').font = italic_font
ws.merge_cells(start_row=row, start_column=start_col, end_row=row, end_column=start_col+1)

row += 1
shareholders = [
    ('PT Multi Artha Pratama (Pengendali)', '84,09%'),
    ('Publik (Non-Warkat)', '15,91%'),
    ('Total Saham Beredar', '18.117.025.598 lembar'),
]
...
row += 1
investor_history = [
    ('Desember 2025', '49.619 (+1.045)'),
    ('November 2025', '48.574 (+1.277)'),
    ('Oktober 2025', '47.297 (-5.005)'),
    ('September 2025', '52.302 (+6.181)'),
    ('Agustus 2025', '46.121 (+5.422)'),
    ('Juli 2025', '40.699 (-1.853)'),
]
...
row += 1
ws.cell(row=row, column=start_col, value='Efektif 9 Oktober 2025').font = italic_font
ws.merge_cells(start_row=row, start_column=start_col, end_row=row, end_column=start_col+1)
...
commissioners = [
    ...
    ('Wakil Presiden Komisaris', 'Phiong Philipus Darma'),
...
print('Profile data added to Excel successfully!')
