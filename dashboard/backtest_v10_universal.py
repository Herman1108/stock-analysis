# -*- coding: utf-8 -*-
"""
Universal Backtest Formula V10
Supports all stocks from zones_config.py
"""
...
def calculate_sl_tp(zh, entry_type, zone_low, zone_high, zone_num, entry_price, params):
    ...
    # Berlaku untuk semua tipe entry (RETEST, BO_HOLD, BO_PULLBACK)
...
def run_backtest(stock_code, params=None, start_date='2024-01-01', verbose=False):
    if params is None:
    ...
    pending_entry_conditions = None  # V10 checklist conditions at entry
        ...
        if pending_entry_type and position is None:
            ...
            if tp <= entry_price:
                ...
                position = {
                    ...
                    'sl': sl,
                    'tp': tp,
                    'entry_conditions': pending_entry_conditions,  # V10 checklist at entry
                }
                if verbose:
                    events_log.append(f"{date_str}: ENTRY {pending_entry_type} Z{pending_entry_zone_num} @ {entry_price:,.0f}")
        ...
        if not frozen and position is None:
            if state == STATE_IDLE and s_low is not None:
                ...
                if s_touch and s_hold and s_not_late and s_from_above and prior_resistance_touched:
                    ...
                    # Save V10 checklist conditions at trigger time
...
def print_report(result):
    ...
    print(f"\n{'='*90}")
    print(f"BACKTEST V10 - {stock_code}")
    print(f"{'='*90}")
    ...
    if trades:
        print(f"\nTRADES:")
        print(f"  {'#':<3} {'Type':<12} {'Zone':<5} {'Entry':<12} {'Price':>10} {'Exit':<12} {'ExitP':>10} {'Reason':<10} {'PnL':>8}")
        print(f"  {'-'*85}")
        for idx, t in enumerate(trades, 1):
            print(f"  {idx:<3} {t['type']:<12} Z{t['zone_num']:<4} {t['entry_date']:<12} {t['entry_price']:>10,.0f} {t['exit_date']:<12} {t['exit_price']:>10,.0f} {t['exit_reason']:<10} {t['pnl']:>+7.1f}%")
...
def run_all_backtests():
    ...
    print("=" * 90)
    print("FORMULA V10 - BACKTEST ALL STOCKS")
    print("=" * 90)
    ...
    # Summary table
    print("\n" + "=" * 90)
    print("RINGKASAN SEMUA SAHAM")
    print("=" * 90)
    print(f"{'Stock':<8} {'Trades':>8} {'Wins':>8} {'Losses':>8} {'WinRate':>10} {'TotalPnL':>12}")
    print("-" * 60)
    ...
    for r in results:
        ...
        print(f"{r['stock_code']:<8} {len(closed):>8} {len(wins):>8} {len(losses):>8} {wr:>9.1f}% {pnl:>+11.2f}%")

    print("-" * 60)
    overall_wr = total_wins/total_trades*100 if total_trades else 0
    print(f"{'TOTAL':<8} {total_trades:>8} {total_wins:>8} {total_trades-total_wins:>8} {overall_wr:>9.1f}% {total_pnl:>+11.2f}%")


if __name__ == '__main__':
    import argparse

    parser = argparse.ArgumentParser(description='Backtest Formula V10')
    ...
    if args.stock.upper() == 'ALL':
        ...
        result = run_backtest(args.stock, verbose=args.verbose)
        if result:
            print_report(result)
